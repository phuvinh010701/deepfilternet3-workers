...
export class RingBuffer {
   ...
   * @return The number of elements written to the queue.
   */
  writeCallback(amount: number, cb: (first: any, second: any) => number | void): number {
    const rd = Atomics.load(this.read_ptr, 0);
    const wr = Atomics.load(this.write_ptr, 0);
   ...
   * @return The number of elements written to the queue.
   */
  writeCallbackWithOffset(amount: number, cb: (storage: any, offset1: number, size1: number, offset2: number, size2: number) => number | void): number {
    const rd = Atomics.load(this.read_ptr, 0);
    const wr = Atomics.load(this.write_ptr, 0);
