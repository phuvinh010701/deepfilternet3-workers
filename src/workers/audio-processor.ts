...
class DeepFilterAudioProcessor extends AudioWorkletProcessor {
  ...
  process(inputList: Float32Array[][], outputList: Float32Array[][], parameters: Record<string, Float32Array>): boolean {
    ...
    if (this._audio_reader.availableRead() >= 128) {
      ...
      console.log(`audioworklet underrun: the worker doesn't enqueue fast enough! available only - ${this._audio_reader.availableRead()}`);
    }
